...
function removeFirstOccurrences(str, value) {
  const indexOfSearchedWord = str.indexOf(value);

  if (indexOfSearchedWord !== -1) {
    return (
      str.slice(0, indexOfSearchedWord) +
      str.slice(indexOfSearchedWord + value.length)
    );
  }
...
function removeLastOccurrences(str, value) {
  const lastIndexOfSearchedWord = str.lastIndexOf(value);

  if (lastIndexOfSearchedWord !== -1) {
    return (
      str.slice(0, lastIndexOfSearchedWord) +
      str.slice(lastIndexOfSearchedWord + value.length)
    );
  }
...
function sumOfCodes(str) {
  ...
  return str
    .split('')
    .reduce((acc, currentValue) => acc + currentValue.charCodeAt(0), 0);
}
...
function formatTime(minutes, seconds) {
  const minutesRes = minutes.toString().padStart(2, '0');
  const secondsRes = seconds.toString().padStart(2, '0');

  return `${minutesRes}:${secondsRes}`;
}
...
function extractNameFromTemplate(value) {
  return value.replace('Hello,', '').slice(1, -1);
}
